...
import { ChartNoAxesCombined, Sparkles, Users, Goal, Target, Brain, ArrowRight } from "lucide-react";
...
export default async function LandingPage() {
  ...
  const averageScore = aggregate._avg.averageScore ? Math.round(aggregate._avg.averageScore) : 92;

  const formatNumber = (num: number) => new Intl.NumberFormat('en-US').format(num);

  return (
    <div className={`w-full ${ibmPlex.className} -mt-8 overflow-hidden`}>
      <TermsModalWrapper />
      ...
      {/* ── HERO SECTION ── */}
      <section className="relative bg-slate-950 text-white pt-24 pb-32 px-4 sm:px-[8%] -mx-4 sm:-mx-6 lg:-mx-8 overflow-hidden">
        ...
        <div className="relative z-10 grid grid-cols-1 lg:grid-cols-[1.1fr_1fr] gap-12 lg:gap-16 items-center w-full max-w-7xl mx-auto">
          ...
          <div className="flex flex-col items-start text-left space-y-8">
            <div className="flex flex-wrap gap-3">
              <div className="flex items-center gap-2 bg-white/5 backdrop-blur-md px-4 py-2 rounded-full border border-white/10 shadow-[0_0_15px_rgba(255,255,255,0.05)]">
                ...
                <span className="text-sm font-medium text-slate-200">สัมภาษณ์แล้ว {formatNumber(totalInterviews > 0 ? totalInterviews : 12450)} ครั้ง</span>
              </div>
              <div className="flex items-center gap-2 bg-white/5 backdrop-blur-md px-4 py-2 rounded-full border border-white/10 shadow-[0_0_15px_rgba(255,255,255,0.05)]">
                <ChartNoAxesCombined size={16} className="text-orange-400" />
                <span className="text-sm font-medium text-slate-200">พึงพอใจ {averageScore}%</span>
              </div>
            </div>
            
            <h1 className="text-5xl sm:text-6xl lg:text-7xl leading-[1.15] font-bold tracking-tight text-white drop-shadow-xl">
              เตรียมความพร้อมสัมภาษณ์งาน<br/>ด้วยพลังของ <span className="text-transparent bg-clip-text bg-linear-to-r from-emerald-400 to-indigo-400">AI</span>
            </h1>
            
            <p className="text-slate-300 text-lg sm:text-xl max-w-xl leading-relaxed">
              จำลองสถานการณ์สัมภาษณ์เสมือนจริง โต้ตอบด้วยเสียง พร้อมวิเคราะห์จุดเด่นจุดด้อยและรับ Feedback แบบเรียลไทม์เพื่อพัฒนาตัวคุณให้พร้อมที่สุด
            </p>
            ...
            <div className="relative bg-slate-900/80 backdrop-blur-xl border border-slate-700/50 rounded-[32px] p-6 shadow-2xl">
              ...
              <div className="bg-slate-800/60 rounded-2xl p-5 border border-slate-700/50">
                <div className="flex justify-between items-end mb-4">
                  <div>
                    <h3 className="text-slate-200 font-bold mb-1">ผลคะแนนล่าสุด</h3>
                    <p className="text-emerald-400 font-medium text-sm">การสื่อสารดีเยี่ยม ตรงประเด็น</p>
                  </div>
                ...
                <div className="w-full bg-slate-900 rounded-full h-2 mb-2">
                  <div className="bg-linear-to-r from-emerald-400 to-indigo-400 h-2 rounded-full" style={{ width: `${averageScore}%` }} />
                </div>
              </div>
            </div>

            {/* Floating User Card */}
            <div className="absolute -left-4 sm:-left-16 bottom-16 bg-slate-900/90 backdrop-blur-xl border border-slate-700/50 py-3 px-4 sm:p-4 rounded-2xl shadow-[0_0_30px_rgba(0,0,0,0.5)] flex items-center gap-3 sm:gap-4 animate-float z-20" style={{ animationDelay: '1.5s' }}>
              <div className="flex -space-x-3">
                <div className="w-8 h-8 sm:w-10 sm:h-10 rounded-full bg-slate-300 border-2 border-slate-900 overflow-hidden"><img src={avatarsToShow[0]} alt="user 1" className="w-full h-full object-cover" /></div>
                <div className="w-8 h-8 sm:w-10 sm:h-10 rounded-full bg-slate-300 border-2 border-slate-900 overflow-hidden"><img src={avatarsToShow[1]} alt="user 2" className="w-full h-full object-cover" /></div>
                <div className="w-8 h-8 sm:w-10 sm:h-10 rounded-full bg-slate-300 border-2 border-slate-900 overflow-hidden"><img src={avatarsToShow[2]} alt="user 3" className="w-full h-full object-cover" /></div>
                <div className="w-8 h-8 sm:w-10 sm:h-10 rounded-full bg-slate-800 border-2 border-slate-900 flex items-center justify-center text-[9px] sm:text-[10px] font-bold text-emerald-400">+{formatNumber(userCount > 0 ? userCount : 5000)}</div>
              </div>
              <div className="text-left">
                <p className="text-xs sm:text-sm font-bold text-white leading-tight">ผู้ใช้งานระบบ</p>
                <div className="flex items-center gap-1.5 mt-1">
                  <span className="w-2 h-2 rounded-full bg-emerald-400 animate-pulse"></span>
                  <p className="text-[10px] sm:text-xs text-emerald-400 font-medium">กำลังเตรียมความพร้อม</p>
                </div>
              </div>
            </div>
        ...
        <div className="max-w-7xl mx-auto">
          <div className="text-center mb-16">
            ...
            <p className="text-slate-500 text-lg max-w-2xl mx-auto">
              ออกแบบกระบวนการฝึกฝนให้เหมือนสนามจริงมากที่สุด เพื่อลดความประหม่าและเพิ่มความมั่นใจในการสัมภาษณ์ของคุณ
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 relative">
            ...
            <div className="relative bg-slate-50 rounded-3xl p-8 border border-slate-100 shadow-sm hover:shadow-xl transition-all duration-300 hover:-translate-y-2 group">
              ...
              <div className="mt-8 flex flex-col items-center text-center">
                ...
                <h3 className="text-xl font-bold text-slate-800 mb-3">ระบุตำแหน่งที่ต้องการ</h3>
                <p className="text-slate-600 leading-relaxed text-sm">
                  เลือกสายงาน ตำแหน่ง ระดับ (Junior/Senior) และวางอธิบายงาน (Job Description) ตัวระบบจะสร้างคำถามให้ตรงจุดโดยอัตโนมัติ
                </p>
              </div>
            </div>
            ...
            <div className="relative bg-slate-50 rounded-3xl p-8 border border-slate-100 shadow-sm hover:shadow-xl transition-all duration-300 hover:-translate-y-2 group">
              ...
              <div className="mt-8 flex flex-col items-center text-center">
                ...
                <h3 className="text-xl font-bold text-slate-800 mb-3">สัมภาษณ์โต้ตอบเสมือนจริง</h3>
                <p className="text-slate-600 leading-relaxed text-sm">
                  พูดโต้ตอบด้วยเสียงผ่านไมโครโฟน AI จะทำหน้าที่เป็นผู้สัมภาษณ์ที่สามารถไล่ฟังสคริปต์และถามเจาะลึก(Deep Dive) ได้เหมือนคนจริงๆ
                </p>
              </div>
            </div>
            ...
            <div className="relative bg-slate-50 rounded-3xl p-8 border border-slate-100 shadow-sm hover:shadow-xl transition-all duration-300 hover:-translate-y-2 group">
              ...
              <div className="mt-8 flex flex-col items-center text-center">
                ...
                <h3 className="text-xl font-bold text-slate-800 mb-3">รับผลและคำแนะนำ</h3>
                <p className="text-slate-600 leading-relaxed text-sm">
                  ดูคะแนนภาพรวม จุดที่คุณทำได้ดี และจุดที่ต้องแก้ไข พร้อมรับแนวทางการตอบแบบ STAR Technique ให้คุณพร้อม 100%
                </p>
              </div>
            </div>
